...
def grid_points(img, nPointsX, nPointsY, border):
    ...
    # todo
    xx, yy = np.meshgrid(
        np.linspace(border, img.shape[0]-border, num=nPointsX, dtype=int), 
        np.linspace(border, img.shape[1]-border, num=nPointsY, dtype=int)
    )
    vPoints = np.vstack((np.array(xx).flatten(), np.array(yy).flatten())).T
    return vPoints
...
def descriptors_hog(img, vPoints, cellWidth, cellHeight):
    ...
    for i in range(len(vPoints)):
        x_0, y_0 = vPoints[i,0] - nCells/2*w, vPoints[i,1] - nCells/2*h
        hogs = []
        for x in range(nCells):
            for y in range(nCells):
                xi , yi = int(x_0 + x*w), int(y_0 + y*h)
                patch_direction = direction[xi:xi+w, yi:yi+h]
                histo, _ = np.histogram(patch_direction, bins = np.linspace(-np.pi, np.pi, num=nBins + 1))
                hogs.append(histo)
        hogs = np.concatenate(hogs)
        descriptors.append(hogs)
...
def bow_histogram(vFeatures, vCenters):
    ...
    histo, _ = np.histogram(Idx, bins = np.arange(N))
    return histo
